...
from odoo.addons.pr_acess_managment.models.domain_prepare import prepare_domain_v2
...
{resolution_info}""".format(operation_error=operation_error, resolution_info=resolution_info)
    ...
    def unlink(self):
        value = self.env['ir.config_parameter'].sudo().search([('key', '=', 'uninstall_pr_acess_managment')],
                                                              limit=1).value
        if not value:
            for rec in self:
                if rec._name:
    ...
    #     value = self.env['ir.config_parameter'].sudo().search([('key','=','uninstall_pr_acess_managment')],limit=1).value
    #     if not value:
    #         if self._name:
