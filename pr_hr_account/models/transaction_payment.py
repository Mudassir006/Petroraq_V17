    ...
    def _onchange_cs_employee_id(self):
        for line in self:
            ...
            if line.debit_cs_employee_id:
                ...
                if line.debit_cs_employee_id.cs_project_id and str(
                        line.debit_cs_employee_id.cs_project_id.id) not in analytic_distribution:
                    analytic_distribution.update({
                        str(line.debit_cs_employee_id.cs_project_id.id): 100.0
                    })
    ...
    def _onchange_credit_cs_employee_id(self):
        for line in self:
            ...
            if line.credit_cs_employee_id:
                # Analytic Distribution
                if line.credit_cs_employee_id.project_id and str(
                        line.credit_cs_employee_id.project_id.id) not in analytic_distribution:
                    analytic_distribution.update({
                        str(line.credit_cs_employee_id.project_id.id): 100.0
                    })
