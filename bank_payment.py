...
class AccountBankPayment(models.Model):
    ...
    def action_draft(self):
        """
        Reset BPV to draft state
        """
        for bank_payment in self:
            # Check if journal entry exists and is not already deleted
            if bank_payment.journal_entry_id and bank_payment.journal_entry_id.exists():
                try:
                    # Unpost if posted
                    if bank_payment.journal_entry_id.state == "posted":
                        bank_payment.journal_entry_id.sudo().button_draft()

                    # Delete the journal entry
                    bank_payment.journal_entry_id.sudo().unlink()
                except Exception as e:
                    _logger.warning(f"Failed to unlink journal entry for {bank_payment.name}: {str(e)}")

            # ✅ CRITICAL: Clear the journal_entry_id field
            bank_payment.journal_entry_id = False
    ...
    def action_post(self):
        """
        Post BPV and create journal entry with validations
        """
        for bank_payment in self:
            # ✅ CHECK 1: Prevent duplicate posting - check if valid JE exists
            if bank_payment.journal_entry_id and bank_payment.journal_entry_id.exists():
                # Journal entry already exists and is not deleted
                if bank_payment.journal_entry_id.state == 'draft':
                    # Check if it's valid before posting
                    total_debit = sum(line.debit for line in bank_payment.journal_entry_id.line_ids)
                    total_credit = sum(line.credit for line in bank_payment.journal_entry_id.line_ids)
                    ...
                    if total_debit == 0 and total_credit == 0:
                        ...
                        bank_payment.journal_entry_id.action_post()
                        bank_payment.state = "posted"
                        bank_payment.accounting_manager_state = "posted"
                        continue
                elif bank_payment.journal_entry_id.state == 'posted':
                    # Already posted
                    bank_payment.state = "posted"
                    bank_payment.accounting_manager_state = "posted"
                    continue
            elif bank_payment.journal_entry_id and not bank_payment.journal_entry_id.exists():
                # Broken reference - clear it
                bank_payment.journal_entry_id = False

            # ✅ CHECK 2: Validate there are approved lines
            approved_lines = bank_payment.bank_payment_line_ids.filtered(lambda l: l.state == "approve")
            if not approved_lines:
                # Get all line states for better error message
                line_states = bank_payment.bank_payment_line_ids.mapped('state')
                raise ValidationError(
                    f"Cannot post {bank_payment.name}: No approved payment lines found.\n\n"
                    f"You have {len(bank_payment.bank_payment_line_ids)} line(s) with states: {', '.join(set(line_states))}\n\n"
                    ...
                    "\n".join([f"  • Line {i + 1}: {line.description or 'No description'} - State: {line.state}"
                               for i, line in enumerate(bank_payment.bank_payment_line_ids)])
                )

            # ✅ CHECK 3: Validate approved amount
            if not bank_payment.approved_amount or bank_payment.approved_amount <= 0:
                raise ValidationError(
                    f"Cannot post {bank_payment.name}: Approved amount must be greater than zero.\n"
                    f"Current approved amount: {bank_payment.approved_amount}"
                )
            ...
            if existing_move:
                ...
                if total_debit == 0 and total_credit == 0:
                    ...
                    _logger.warning(
                        f"BPV {bank_payment.name}: Deleted existing zero-amount journal entry {existing_move.name}"
                    )
                else:
                    # Valid JE exists, link it
                    bank_payment.journal_entry_id = existing_move.id
                    if existing_move.state == 'draft':
                        existing_move.action_post()
                    bank_payment.state = "posted"
                    bank_payment.accounting_manager_state = "posted"
                    _logger.info(
                        f"BPV {bank_payment.name}: Linked to existing journal entry {existing_move.name}"
                    )
                    continue

            # ✅ CHECK 5: Validate we have payment lines
            if not bank_payment.bank_payment_line_ids:
                raise ValidationError(
                    f"Cannot post {bank_payment.name}: No payment lines found."
                )
            ...
            if journal_entry_id:
                ...
                # Create credit line (bank/payment account)
                credit_line = move_line.create(
                    bank_payment.prepare_credit_move_line_vals(move_id=journal_entry_id)
                )
                ...
                if debit_amount == 0 or credit_amount == 0:
                    ...
                    raise ValidationError(
                        f"Cannot post {bank_payment.name}: Journal entry would have zero amount.\n"
                ...
                if abs(debit_amount - credit_amount) > 0.01:  # Allow small rounding differences
                    journal_entry_id.unlink()
                    raise ValidationError(
                        f"Cannot post {bank_payment.name}: Journal entry is not balanced.\n"
                ...
                try:
                    ...
                    journal_entry_id.unlink()
                    raise ValidationError(
                        f"Failed to post journal entry for {bank_payment.name}: {str(e)}"
                    )
    ...
    def prepare_credit_move_line_vals(self, move_id=False):
        """Prepare credit line values for BPV"""
        self.ensure_one()
...
class AccountBankPaymentLine(models.Model):
    ...
    def prepare_debit_move_line_vals(self, move_id=False):
        """Prepare debit line values for BPV"""
        self.ensure_one()

        line_vals = {
            "account_id": self.account_id.id,
            "partner_id": self.partner_id.id if self.partner_id else False,
            "name": self.description if self.description else f"Debit Line For {self.bank_payment_id.name}",
            "analytic_distribution": self.analytic_distribution if self.analytic_distribution else False,
    ...
    def prepare_debit_tax_move_line_vals(self, move_id=False):
        """Prepare debit tax line values for BPV"""
        self.ensure_one()
        ...
        line_vals = {
            ...
            "partner_id": self.partner_id.id if self.partner_id else False,
            "name": self.description if self.description else f"Debit Tax Line For {self.bank_payment_id.name}",
            "analytic_distribution": self.analytic_distribution if self.analytic_distribution else False,
            "cs_project_id": self.cs_project_id.id if self.cs_project_id else False,
            'display_type': 'tax',
